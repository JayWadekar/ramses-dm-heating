...
subroutine rho_fine(ilevel,icount)
  ...
  do ind=1,twotondim
     ...
     do i=1,active(ilevel)%ngrid
        ...
        rho_dm(active(ilevel)%igrid(i)+iskip)=0.0D0
     end do
  ...
  do icpu=1,ncpu
     do ind=1,twotondim
        ...
        do i=1,reception(icpu,ilevel)%ngrid
           ...
           rho_dm(reception(icpu,ilevel)%pcomm%igrid(i)+iskip)=0.0D0
           phi(reception(icpu,ilevel)%pcomm%igrid(i)+iskip)=0.0D0
#else
           rho(reception(icpu,ilevel)%igrid(i)+iskip)=0.0D0
           rho_dm(reception(icpu,ilevel)%igrid(i)+iskip)=0.0D0
  ...
  do ibound=1,nboundary
     do ind=1,twotondim
        ...
        do i=1,boundary(ibound,ilevel)%ngrid
           ...
           rho_dm(boundary(ibound,ilevel)%igrid(i)+iskip)=0
        end do
     end do
  end do
